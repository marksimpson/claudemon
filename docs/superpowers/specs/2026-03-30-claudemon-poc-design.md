...
| IDLE | Waiting for user input | `Notification` hook with `idle_prompt` |
...
Three hooks, all handled by a single script (`~/.claude/claudemon/hook.sh`):
...
{
  "session_id": "718bc7c3-...",
  "hook_event_name": "SessionStart|Notification|UserPromptSubmit",
  "cwd": "/path/to/project"
}
```
...
/usr/local/bin/claudemon   # CLI script (or ~/bin, wherever Mark prefers)
```
